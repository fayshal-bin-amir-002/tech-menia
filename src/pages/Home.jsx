...
const Home = () => {
    ...
    useEffect(() => {
        setLoading(true);
        fetch(`${import.meta.env.VITE_BASE_URL}/products?page=${currentPage - 1}&search=${search}&brand=${brand}`)
            .then((res) => res.json())
            .then((data) => {
        ...
        <div className="py-6 md:py-8 lg:py-12">
            <div>
                <form onSubmit={handleSearch} className="flex justify-center mb-6">
                    <label className="input input-bordered flex items-center gap-2 w-full lg:w-1/2 mx-auto">
                        <input type="text" name="search" className="grow w-full" placeholder="Search product name" />
                ...
                <div className="drawer-side">
                    ...
                    <ul className="menu bg-base-200 text-base-content min-h-full w-80 p-4">
                        ...
                        <div className="mb-5">
                            ...
                            <form>
                                {
                                    priceRanges.map((price) => <div key={price.price} className="flex items-center gap-2 mb-1"><input type="checkbox" className="checkbox" onChange={(e) => e.target.checked ? setPriceRange([...priceRange, price.price]) : setPriceRange(priceRange.filter((r) => r !== price.price))} /> <span>{price.price}</span></div>)
                                }
                            </form>
                        </div>
...

            {
                loading ? <LoadingSpinner></LoadingSpinner> :
                    products ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
